...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        parser.add_argument(
            "--name", type=str, required=True, help="Name of the API client"
        )
        parser.add_argument(
            "--type",
            type=str,
        ...
        parser.add_argument(
            "--description", type=str, default="", help="Description of the client"
        )
        parser.add_argument(
            "--rate-limit",
            type=int,
    ...
    def handle(self, *args, **options):
        ...
        if APIClient.objects.filter(name=name).exists():
            self.stdout.write(self.style.ERROR(f'Client "{name}" already exists!'))
            return
        ...
        self.stdout.write(self.style.SUCCESS(f"✓ API Client created successfully!"))
        self.stdout.write(f"\nClient Details:")
